...
N 代表数据集中所有文档的数量。分数![](img/latex2.png)就是所谓的逆文件频率。如果一个单词出现在许多文档中，则其逆文档频率接近 1。如果单词出现在较少文档中，则逆文档频率要高得多。
...
![Figure 4-2: 图 4-1 中四个句子的 Tf-idf 表示](img/4-2.png)
...
![Figure 4-3: sigmoid 函数](img/4-3.png)
...
![Figure 4-4: 分类器精度在每个特征集和正则化设置下的分布。 准确度是以 5 折交叉验证的平均准确度来衡量的](img/4-4.png)

<center><h5>Figure 4-4: 分类器精度在每个特征集和正则化设置下的分布。 准确度是以 5 折交叉验证的平均准确度来衡量的</h5></center>
...
![Figure 4-5: 包含 5 个文档 7 个单词的文档-词汇矩阵](img/4-5.png)

<center><h5>Figure 4-5: 包含 5 个文档 7 个单词的文档-词汇矩阵</h5></center>
...
列空间被定义为所有列向量的线性组合：![](img/latex4.png)。比方说，特征缩放用一个常数倍来替换一个列向量，![](img/latex8.png)。但是我们仍然可以通过用![](img/latex5.png)来替换![](img/latex6.png)，生成原始的线性组合。看起来，特征缩放不会改变列空间的秩。类似地，特征缩放不会影响空间的秩，因为可以通过反比例缩放权重向量中的对应条目来抵消缩放的特征列。

但是，仍然存在一个陷阱。 如果标量为 0，则无法恢复原始线性组合;![](img/latex7.png)消失了。 如果该向量与所有其他列线性无关，那么我们已经有效地缩小了列空间并放大了零空间。
