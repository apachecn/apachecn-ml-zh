...
![Topology](images/image_3722_03_03.jpg)
...
![Training](images/image_3722_03_04.jpg)
...
![Applying a dA](images/image_3722_03_05.jpg)
...
在本章前面，我们描述了自动编码器如何通过映射(如![Applying a dA](images/image_B03722_03_01Fpng.jpg))在可见层和隐藏层之间转换。为了实现这种转换，需要定义`W`、`b`、`W'`和`b'`与前面描述的自动编码器参数、`bhid`、`bvis`和`W`相关。`W'`和`b'`在以下代码中称为`W_prime`和`b_prime`:
...
接下来，我们获得隐藏值。这是通过执行等式![Applying a dA](images/image_B03722_03_02Fpng.jpg)以获得 *y* (隐藏值)的代码来完成的。为了获得自动编码器的输出( *z* )，我们通过使用先前定义的`b_prime`和`W_prime`执行![Applying a dA](images/image_B03722_03_03Fpng.jpg)的代码重建隐藏层:
...
最后缺少的部分是成本更新的计算。我们之前回顾了一个成本函数，一个简单的平方误差度量:![Applying a dA](images/image_B03722_03_04Fpng.jpg)。让我们使用这个成本函数来计算我们的成本更新，基于输入(`x`)和重构(`z`):
