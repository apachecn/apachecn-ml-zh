...
在训练中，我们学习了朴素贝叶斯模型，这是当我们已经知道特征![](assets/23913d80-92b7-4d12-b60e-a9b39d0dbaf3.png)和![](assets/ee1420a4-5a95-4e74-9dfd-7cdac366a3f4.png)时![](assets/8eafb9c2-c8cb-4b41-af43-61f72360fd86.png)类的概率。这个概率写成![](assets/3fd2170b-30fe-4950-b30e-29c3decf01fe.png)。

由于我们不能直接估计![](assets/4962d4de-e8c3-4881-8a4c-de76668a2ba8.png)，我们应用了一个技巧，这个技巧被贝叶斯发现:

![](assets/8d80594e-e1d3-4a7a-998b-034e3cf321ea.png)

如果我们将![](assets/dfa4758b-4043-449a-b25c-00bdc7c2e023.png)替换为“可怕”和“疯狂”的概率，并将![](assets/37a024b9-8049-4fbb-87e2-729b7432bd15.png)视为我们的![](assets/fa375f43-2eca-4f3c-b2f7-146b20639716.png)类，我们会得到一个关系，帮助我们稍后检索属于指定类的数据实例的概率:

![](assets/b439aec2-f5d0-478e-8c42-fbf12841e3ab.png)

这允许我们通过其他概率来表达![](assets/d6bfcd56-39a2-4e24-8fdb-6e8c2c02164b.png):

![](assets/2308917c-8730-46ed-8a4f-f42c21dca9cb.png)

我们也可以这样描述:

![](assets/ef87867f-5c30-4fc0-80b1-1e9471150efb.png)

*在先*和*证据*很容易确定:

*   ![](assets/3c19f65e-3993-43e5-8c0a-c5324d366aa0.png)是不知道数据的![](assets/131d5794-3f70-4e0f-9efb-a94a0bd27f60.png)类的先验概率。我们可以通过简单地计算属于该特定类的所有训练数据实例的分数来估计这个数量。
*   ![](assets/0c1f310c-0e92-444e-a41b-dc0f85f27442.png)是特征![](assets/0073006a-1ec8-408b-bc0d-2b0ad5fd41dc.png)和![](assets/8744b0ac-8e86-49ae-ac0f-808eb21fd962.png)的证据或概率

棘手的部分是可能性![](assets/dd3acf05-c9e0-4c76-8549-09cf62910b5c.png)的计算。如果我们知道数据实例的类是![](assets/b0d34917-a230-4dfe-8def-09eb26064571.png)，那么这个值描述了看到![](assets/2b6fcd86-26a2-4e62-8d47-56ff064836f2.png)和![](assets/cd31f96f-53a5-40ac-80bf-5d802a217d5d.png)特征值的可能性。要估计这一点，我们需要做一些思考。

# 天真

从概率论中，我们还知道以下关系:

![](assets/1bba6d06-c620-47ef-8e36-1152a087db66.png)

然而，这本身并没有多大帮助，因为我们用另一个难题(估计![](assets/b6686b94-6f17-460b-8263-ef56401738ea.png))来处理一个难题(估计![](assets/531b440c-289d-4376-bba2-d76edbfe2c8d.png))。

然而，如果我们天真地假设![](assets/cf4d13e4-c636-4ad5-aa37-521df0a01e0e.png)和![](assets/6c6573fd-fb44-4818-b47d-43752cd7e4e4.png)相互独立，![](assets/121b8827-e3e4-49c2-846c-c072c2fbce5d.png)简化为![](assets/935e439d-e962-4158-abc2-113a97789ff8.png)，我们可以这样写:

![](assets/0db8c5e7-bb3c-4737-a3a3-0b93ada7b2c1.png)

把所有的东西放在一起，我们得到了以下非常容易管理的公式:

![](assets/e58cf94b-f8ba-470e-b9ce-15fa4a0d4559.png)
...
![](assets/1a41a64d-1616-4118-a83f-4d7405c22123.png)

然后选择![](assets/7bb1a340-f914-4f14-a3cb-61acca2fda5d.png)类作为概率较高的。

至于两个类，分母![](assets/3154b9a8-ace6-45f2-a510-8b509559817a.png)是一样的，我们可以简单的忽略它，不改变胜者类。

然而，请注意，我们不再计算任何真实的概率。相反，我们正在估计哪一类更有可能给出证据。这是朴素贝叶斯如此稳健的另一个原因:它对真实概率不感兴趣，只对信息感兴趣，哪个类更有可能。简而言之，我们可以这样写:

![](assets/741f4807-63aa-4c05-bb52-e954b7b84a38.png)

简单来说就是我们在计算![](assets/2bbfada1-06d6-4050-957d-43357b695c81.png) ( *pos* 和 *neg* 的所有类在 *argmax* 之后的部分，并返回得到最高值的类。
...
![](assets/85e27f89-c306-45ea-9d46-5cb1b0d89037.png)

![](assets/4e549f58-6b22-43c6-a186-66ab9fb69d73.png)

这意味着，在对推文本身一无所知的情况下，假设推文是正面的是明智的。

我们仍然缺少![](assets/995572e0-38c9-4439-9286-8b271f54ceb9.png)和![](assets/841ea72a-4f1c-4d67-8b67-4afcab86e2a0.png)的计算，这是两个特征![](assets/bc6de720-e7b3-4898-9124-a97b133df0ec.png)和![](assets/70da9e1e-e044-48d8-a30c-f16cc187c94c.png)的概率，在![](assets/cc071e74-95a0-4f83-93a7-fda4ffd3348b.png)类中有条件。

这是通过我们已经看到具体特征的推文数量除以已经用![](assets/23831d39-d7f6-45b7-b759-aea715c1b641.png)类标记的推文数量来计算的。假设我们想知道在推文中看到 awesome 发生的概率，知道它的类是正的，我们将有:

![](assets/7e962c8a-9893-4463-8890-76aafd83f3fb.png)

这是因为在四条积极的推文中，有三条包含了“棒极了”这个词。显然，在正面推文中没有令人敬畏的可能性是相反的:

![](assets/0d6be141-abc2-4824-b486-a0059e55769e.png)

同样，对于其余部分(省略一个词没有出现在推文中的情况):

![](assets/12a60172-85fe-42dd-b3bd-de827bbc226a.png)

![](assets/e78b5164-9239-4211-a680-37e01b78b857.png)

![](assets/dc6ff4af-2f44-474d-975f-d0727a82a541.png)

为了完整起见，我们还将计算证据，以便我们可以在下面的示例推文中看到真实概率。对于![](assets/95bcf2cf-bbff-416e-b2e6-37722859b4a1.png)和![](assets/32e91d03-f8cf-4f8e-a143-95613f28d320.png)两个具体数值，我们可以计算出如下证据:

![](assets/0c97d14e-d058-4d9b-b0c8-6de5381865d5.png)

这导致以下值:

![](assets/70ef5d4c-adb9-4646-b741-083b2c5e0979.png)

![](assets/e7f6b1a3-6376-4e17-8cf9-4d1601b80d63.png)

![](assets/5be4b66a-915c-4791-86a8-ca85839b40cd.png)

现在我们有了对新推文进行分类的所有数据。剩下的唯一工作就是解析这条推文，分析它的特点:

| **推文** | **![](assets/e76a4939-5e4c-4077-a002-851dd3dc01f7.png)** | **![](assets/9f25dc9c-388d-454e-9dc1-34b93c6a3401.png)** | **类概率** | **分类** |
| 可怕的 | one | Zero | ![](assets/3561cf10-d4ed-400f-8260-c00dbc55a4fb.png)![](assets/86aabea3-5033-4d8f-b808-608e977bfb07.png) | 积极的 |
| 疯狂的 | Zero | one | ![](assets/9199b950-65f7-4824-8169-7d7e58cd21bc.png)![](assets/e699dd65-abcb-434a-ad79-83f6283d8790.png) | 否定的；消极的；负面的；负的 |
| 可怕的疯狂 | one | one | ![](assets/ae295d4a-5823-4299-be50-42ecb7247135.png)![](assets/99e14690-b5ad-4974-9615-4b4f6b934549.png) | 积极的 |
...
这是一种非常简单的技术，可以将一个要素添加到所有要素实例中。它有一个潜在的假设，即使我们没有在整个语料库中看到一个给定的单词，我们的推文样本仍然有可能不包括这个单词。因此，通过加一平滑，我们假装我们比实际看到的更多地看到了每一个事件。这意味着我们现在不用计算![](assets/864e95d0-5ac7-43b2-aa31-fa4e2fdf3783.png)，而是做![](assets/e0f824bf-dc21-43c1-b294-889607073518.png)。

为什么分母要加 2？因为我们有两个特点:牛逼和疯狂的发生。因为我们为每个特征加 1，我们必须确保最终结果还是一个概率。事实上，我们得到 1 作为总概率:

![](assets/836d737e-1913-4b0a-8ca9-5ea6cd9ecf06.png)
...
![](assets/17beeca3-08df-4b9f-bc54-d269b54549a0.png)

如果我们将此公式应用于我们的案例，我们会得到以下结果:

![](assets/1e1cb84b-fa33-4cce-8761-9f908c735e94.png)

由于概率在 **0** 和 **1** 之间的区间内，概率的对数在-和 0 之间的区间内。不要为此烦恼。更高的数字仍然是正确类别的一个更强有力的指标——只是它们现在是负数:

![](assets/817d158c-4321-417a-a6cb-52cdaf5fae35.png)

但是有一个警告:我们实际上没有公式命名者的日志(分数的顶部)。我们只有概率的乘积。幸运的是，在我们的例子中，我们对概率的实际值不感兴趣。我们只是想知道哪个类的后验概率最高。我们很幸运，因为如果我们发现以下情况:

![](assets/c70bc926-dfaf-4680-8a71-5bd485621990.png)

那么我们也将永远拥有:

![](assets/177f7b61-dbcb-4557-96ad-97a3790b213c.png)。

快速看一下前面的图表，可以发现曲线是严格单调递增的，也就是说，当我们从左向右时，它总是向上的。让我们把这个放在前面提到的公式中:

![](assets/0bac80f0-cb6c-45dd-89bc-d7184fbcce8a.png)

这将最终检索出两个特性的公式，这两个特性将为我们提供最好的类，也是我们将在实践中看到的真实数据:

![](assets/173cf306-de45-441f-902f-a40a4276db83.png)

当然，如果只有两个特性，我们将不会非常成功，所以让我们重写它，以允许任意数量的特性:

![](assets/95f4800a-e312-4aff-9a29-eb9962264cd9.png)
...
![](assets/ef3ddc7f-04df-4ef5-b426-23c5b02bb1ca.png)
...
![](assets/311bdfee-645f-485a-b2ef-55afe9dea8bb.png)
...
![](assets/b317e87d-149d-4d5a-b65a-962f5fa46a57.png)
...
![](assets/5dfd2ece-2360-4b14-9b9a-82a556368457.png)
...
由于我们刚刚请求了一个参数，扫描![](assets/67b81b2d-4b40-44a9-af28-f4ac47bf78f1.png)参数组合，每个参数组合被训练 10 次:
...
![](assets/e9922489-d2ec-41fe-bc6a-9c1486f0c1d1.png)

看看下面的情节:

![](assets/e3cd2960-6509-4ed3-870b-2c4d446b280a.png)
...
![](assets/459f23da-166f-4a96-8a98-f168007bfe93.png)
...
![](assets/02245dee-e5e0-4ac6-b82f-0603d016616e.png)
