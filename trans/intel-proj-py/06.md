...
![](assets/eaa4391a-b8cf-42ce-bad4-c4977e83bdbc.png)
...
![](assets/e3c4c366-a9dc-4d53-94a7-6286e1708659.png)
...
基于潜在因子分解的过滤推荐方法试图通过分解评级来发现代表用户和项目简档的潜在特征。与基于内容的过滤特征不同，这些潜在特征是不可解释的，并且可以表示复杂的特征。例如，在电影推荐系统中，潜在特征之一可能代表幽默、悬疑和浪漫的特定比例的线性组合。通常，对于已经评级的项目，用户 *i* 对项目 *j* 给出的评级*r<sub>ij</sub>T3】可以表示为![](assets/08ca7c40-3d15-4f76-9361-0ecf5097bba3.png)。其中 *u <sub> i </sub>* 是基于潜在因素的用户简档向量， *v <sub> i </sub>* 是基于相同潜在因素的项目向量:*

![](assets/719736b5-d23e-49ca-b6b2-bfcda9d9657a.png)
...
![](assets/7eb81448-228b-4859-a03a-7e499d84b422.png)
...
![](assets/1c084aa4-07bd-45cb-bdc1-cbd02d52565c.png)
...
![](assets/f686394a-db58-4ac8-937b-d66b473a96b7.png)
...
![](assets/9d4768be-e18c-43cf-85d0-a9a80c3bfbc1.png)
...
进行基于潜在因素的推荐最常见的方式是将用户简档和偏好定义为 *u <sub>i</sub> ∈ R <sup>k</sup>* 和 *b <sub>i</sub> ∈ R* ，将项目简档和偏好定义为*v<sub>I</sub>∈R<sup>k</sup>*和*b<sub>j</sub>**∈11 用户 *i* 对项目 *j* 提供的等级![](assets/0cdbff49-855e-4e8b-83d9-9e15fd915288.png)定义如下:*

![](assets/2b104cdf-38fe-4cd4-88eb-dfbebd434b43.png)

是所有收视率的全局均值。

用户简档和项目简档然后通过最小化在预测由用户评级的所有项目的评级时的误差平方和来确定。要优化的平方误差损失可以表示如下:

![](assets/90b70a77-88f0-4cb3-b210-094c8e35ca35.png)

*I <sub>ij</sub>* 是一个指标功能，如果用户 *i* 有一个评级项目 *j，则该指标功能为 1；*否则为零。

相对于用户和项目简档的参数，成本被最小化。通常，这种优化会导致过度拟合，因此用户的规范和项目配置文件在成本函数中被用作规范，如下所示:

![](assets/49b382af-40f5-4518-a77f-e04a3c6ad7cc.png)
...
<sup>![](assets/f17a6b6e-bf3f-4fd9-a849-0d012a9af206.png)</sup>

Figure 6.6: Restricted Boltzmann machines for binary data

为了刷新我们的记忆，前面的图(*图 6.6* )是一个有 *m* 输入或可见单位的 RBM。这是投射到一个隐藏层与 *n* 单位。给定可见层输入![](assets/30b2a809-9549-4b21-b876-5bc31a253f97.png)，隐藏单元相互独立，因此可以如下采样，其中![](assets/71f41038-5216-48cd-9474-3d36fd1c8758.png)表示 sigmoid 函数:

![](assets/e4c4d443-0ad5-4a56-a6ac-73c05bcca858.png)

类似地，给定隐藏层激活![](assets/3353a514-2aab-4315-b1c9-9657d1980099.png)，可见层单元是独立的，可以如下采样:

![](assets/8b9a6a5c-dafb-4b91-b89d-676a37ce40ce.png)

RBM 的参数是可见层单元 *i* 和隐藏层单元 *j* 之间的广义权重连接 *w <sub>ij</sub> ∈ W <sub>m x n</sub>* ，可见单元 *i* 处的偏置 *c <sub>i</sub> ∈ b* ，以及偏置 *c <sub>j</sub> ∈ c*

RBM 的这些参数是通过最大化可见输入数据的可能性来学习的。如果我们用![](assets/6e1f5d8d-3ab5-4e9f-b655-406ea08c4f5d.png)表示组合参数集，并且我们有一组 *T* 训练输入数据点，那么在 RBM，我们尝试最大化似然函数:

![](assets/6498b6ad-daea-4a98-a082-ff764b9de960.png)

我们通常不使用乘积形式，而是最大化可能性的对数，或者最小化对数可能性的负值，以使函数在数学上更加方便。如果我们将对数似然的负值表示为成本函数 *C* ，那么:

![](assets/922196ec-aafc-4e51-8ac9-e92674283245.png)

成本函数通常通过梯度下降最小化。成本函数相对于参数的梯度由预期项组成，表示如下:

![](assets/b76195e1-c984-4653-afb4-b60f0db659c0.png)

![](assets/b157601c-6eac-4503-953e-66e71470d453.png)

![](assets/2f08b2c0-b2cf-4e04-abda-de583497996c.png)

术语![](assets/cc978ebd-0c2f-4826-9077-62919760b470.png)表示隐藏单元和可见单元的联合概率分布上任何给定量的期望。此外，![](assets/22e2a62c-c438-4531-b536-8ef4b47a74f7.png)表示给定可见单位 *v* 的采样隐藏层输出。在梯度下降的每次迭代中计算联合概率分布的期望值在计算上是困难的。我们求助于一种叫做**对比发散**的智能方法来计算期望值，这将在下一节讨论。

# 对比分歧

计算联合概率分布期望值的方法之一是通过吉布斯抽样从联合概率分布中产生大量样本，然后将样本的平均值作为期望值。在吉布斯抽样中，联合概率分布中的每个变量都可以抽样，条件是其余变量。由于可见单元是独立的，给定隐藏单元，反之亦然，您可以将隐藏单元采样为![](assets/86849800-744f-4771-96a1-94246e7b9332.png)，然后给定隐藏单元的可见单元激活为![](assets/0738e0a3-6a59-46f7-96a1-dea1b0bc29f8.png)。然后，我们可以将样本![](assets/c9b254f1-f757-4479-8744-0fed6b0d930d.png)作为从联合概率分布中采样的样本。这样，我们可以生成大量的样本，比如说 *M* ，取它们的平均值来计算期望的期望值。然而，在梯度下降的每一步中进行如此广泛的采样将使训练过程慢得不可接受，因此，我们不是在梯度下降的每一步中计算许多样本的平均值，而是从联合概率分布中仅生成一个样本，该样本应该代表整个联合概率分布上的期望:

![](assets/e1fb8a28-45d4-4ebd-b17c-861618cfd639.png)

Figure 6.7: Contrastive divergence illustration

从上图(*图 6.7* )的图示可以看出，我们从看到的可见输入 *v <sup>(t)</sup>* 开始，基于条件概率分布 *P(h/v = v <sup>(t)</sup> )* 对隐藏层激活![](assets/f6ddc0c8-b917-44aa-a6c8-dadd737358ab.png)进行采样。再次，使用条件概率分布*P(v/h = h<sup>′</sup>)*，我们对*v<sup>′</sup>*进行采样。基于条件概率分布*P(h/v = v<sup>′</sup>)*的隐藏单元下一次采样给我们![](assets/2f29e0d4-e0c2-4c48-aa9f-9f3b52890d1b.png)，然后使用![](assets/af67a34a-3350-44c1-b5f8-9bb47f2c265a.png)采样可见单元激活给我们![](assets/71ba3b2e-6fa9-482f-b01d-a3bd8bdac420.png)。样本![](assets/bbe647d9-d45c-448f-8169-04ef6cc6a007.png)取 *v* 和 *h* 整个联合概率分布的代表样本，即![](assets/a13d0854-7fa0-4a6f-a01b-d277a377c7eb.png)。同样用于计算任何包含 *v* 和 *h* 的表达式的期望值。这种取样过程被称为对比发散。
...
![](assets/430c8607-253c-4471-8fdd-cf581201fc71.jpg)

Figure 6.8a: RBM for collaborative filtering User A view

关于**用户 B** 的 RBM 评级如下:

![](assets/d560a0d6-1532-47c4-b43c-d9f44e1f5835.jpg)
...
![](assets/e65a7129-a15e-4668-9318-a0679190ba63.png)

这里， *m = M * k.*

与传统的 RBM 不同，在给定隐藏层激活的情况下，该网络中可见层的二进制单位不能被独立采样。关于电影等级的每一个 *k* 二进制单位通过 *k 路软最大值*激活功能联系在一起。如果给定隐藏单元的特定电影的可见单元的输入是![](assets/ea7938ea-796c-4ab5-af23-84d6aae4513a.png)，则电影的等级*l*I 的一般输入计算如下:

![](assets/ebac218a-2831-48b7-ad7b-c718d8fcc48e.png)

这里， *(i - 1)k + l* 是电影 *i* 的可视单元的索引，用于排名 *l* 。类似地，任何特定电影的可视单元都可以根据软最大值函数给出的概率进行采样，如下所示:

![](assets/4486f0fa-b798-43eb-b87e-0522b99cba86.png)
...
![](assets/c23ff63c-d7c9-4531-b0b0-19c04bd9678e.png)

这里:

![](assets/6f025037-360b-4e15-a688-5b7f0d1f510f.png)

![](assets/a673373a-c431-4db8-849e-0fd1586b0482.png)
...
![](assets/99f2041e-3c09-4503-b01b-c15bcdd1f6b3.png)
