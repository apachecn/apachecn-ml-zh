...
与其他对给定任务容量过大的模型一样，**正则化**可以通过约束自动编码器的学习过程并迫使其产生有用的表示来帮助解决**过度拟合**挑战（例如，参见*第 7 章*、*线性模型——从风险因素到回报预测*、线性模型的正则化以及*第 17 章*、*交易深度学习*、神经网络）。理想情况下，我们可以精确地将模型的容量与数据分布的复杂性相匹配。在实践中，最优模型通常将（有限的）过剩产能与适当的正则化相结合。为此，我们在训练目标中添加了一个稀疏性惩罚![](img/B15439_20_003.png)，该惩罚取决于编码层*h*的权重：
...
到目前为止，我们已经讨论过的自动编码器设计用于在容量受限的情况下再现输入。另一种方法是用损坏的输入![](img/B15439_20_005.png)训练自动编码器，以输出所需的原始数据点。在这种情况下，自动编码器将损失*L*降至最低：
